...
bool isF(float dirz[5]){ //SEEMS TO WORK WITH SOME DEGREE OF CORRECTNESS
	bool check = false;
	float add = dirz[0] + dirz[1];
	float add1 = dirz[2] + dirz[3] + dirz[4];
	std::cout << "isF VALUE " << add << " " << add1 << std::endl;
	if(add >= -.2 && add <= .3 && add1 >= 2.6){
		check = true;
	}
	return check;
}
